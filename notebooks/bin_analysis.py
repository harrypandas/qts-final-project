...
def run_final_strategy(
    ...
    def meltdown_function(day_):
        ...
        nonlocal meltdown_first_has_invested, meltdown_first_invest_day
        nonlocal meltdown_first_invest_i, meltdown_first_long_count, meltdown_first_short_count
        nonlocal current_capital_no_tc
        close_pnl = 0.0
        sum_close_notional = 0.0
        for tkr, pos_ in list(current_positions.items()):
            side = pos_["side"]
    ...
    if len(first_invest_ix) < 2:
        ...
        risk_summary = {
            ...
            "initial_short_count": 0,
            "beta": 0.0,
            "annualized_return_on_capital": 0.0
        }
        df_trades = pd.DataFrame(trade_records).sort_values("date") if trade_records else pd.DataFrame()
        df_risk = pd.DataFrame({"metric": list(risk_summary.keys()), "value": list(risk_summary.values())})
        if not plot:
    ...
    if spy_df is not None:
        ...
        if len(merged_port) > 1:
            cov_ = np.cov(merged_port["ret"], merged_port["spy_ret"])[0, 1]
            var_m = np.var(merged_port["spy_ret"])
            if var_m > 1e-12:
                beta_ = cov_ / var_m
            dmask = merged_port["spy_ret"] < 0
            df_down = merged_port[dmask]
            if len(df_down) > 1:
                cov_down = np.cov(df_down["ret"], df_down["spy_ret"])[0, 1]
                var_down = np.var(df_down["spy_ret"])
                if var_down > 1e-12:
                    ds_beta = cov_down / var_down
    final_capital_value = df_portfolio["portfolio_val"].iloc[-1]
    initial_capital_value = df_portfolio["portfolio_val"].iloc[start_ix]
    final_pnl_net = final_capital_value - initial_capital_value
    final_portfolio_value_no_tc = df_portfolio["portfolio_val_no_tc"].iloc[-1]
    initial_capital_value_no_tc = df_portfolio["portfolio_val_no_tc"].iloc[start_ix]
    final_pnl_no_tc = final_portfolio_value_no_tc - initial_capital_value_no_tc
    n_trading_days = len(sub_df)
    if n_trading_days < 1:
        n_trading_days = 1
    aroc = (1.0 + tot_return) ** (252.0 / n_trading_days) - 1.0
    if meltdown_first_invest_i > 1:
        wait_for_lookback_str = "Yes"
    else:
        wait_for_lookback_str = "No"
    ...
    risk_summary = {
        ...
        "initial_short_count": meltdown_first_short_count,
        "beta": beta_,
        "annualized_return_on_capital": aroc
    }
    df_risk = pd.DataFrame({"metric": list(risk_summary.keys()), "value": list(risk_summary.values())})
    if not plot:
        return df_portfolio, df_trades, df_risk
    ...
    label_map = {
        ...
        "initial_short_count": "Initial Short Count",
        "beta": "Beta",
        "annualized_return_on_capital": "Annualized Return on Capital"
    }
    integer_metrics = {
        "total_days",
        "total_rebalances",
...
def plot_tail_risk(df_portfolio, df_trades, df_risk):
    sub_df = df_portfolio[df_portfolio["ret"].notna()].copy()
    if sub_df.empty:
        return
    returns = sub_df["ret"]
    ...
    tail_mask = x_vals <= var_05
    x_tail = x_vals[tail_mask]
    y_tail = y_vals[tail_mask]
    if len(x_tail):
        fig.add_trace(
            go.Scatter(
                x=np.concatenate([x_tail, x_tail[::-1]]),
                y=np.concatenate([y_tail, [0]*len(y_tail)]),
                fill="toself",
        ...
        fig.add_shape(
            ...
            x1=var_05,
            y0=0,
            y1=float(density(var_05)),
            line=dict(color="red", width=2, dash="dash")
        )
        fig.add_annotation(
            x=var_05,
            y=float(density(var_05)),
            xanchor="left",
            text=(
                f"5% VaR ≈ {var_05:.2%}<br>"
                f"5% CVaR ≈ {cvar_05:.2%}"
    ...
    fig.update_layout(
        title=(
            "Daily Returns (Smoothed) - Tail Risk Focus For Long-only Top 80% Quantile Returns<br>"
            "<sup>"
            "</sup>"
        ),
        xaxis_title="Daily Returns (decimal, e.g. 0.02 = 2%)",
        yaxis_title="Density",
        template="plotly_white",
        title_x=0.5,
...
def plot_top_drawdowns(df_portfolio, top_n=3):
    if df_portfolio.empty or "portfolio_val" not in df_portfolio.columns:
        return

    df_portfolio = df_portfolio.sort_values("date").copy()
    df_portfolio["rolling_max"] = df_portfolio["portfolio_val"].cummax()
    df_portfolio["drawdown"] = (
        df_portfolio["portfolio_val"] - df_portfolio["rolling_max"]
    ) / df_portfolio["rolling_max"]
    ...
    for i in range(len(df_portfolio)):
        dd_i = df_portfolio["drawdown"].iloc[i]
        if not in_drawdown:
            if dd_i < 0:
                in_drawdown = True
                peak_idx = i - 1 if i > 0 else i
        else:
            if dd_i == 0 or i == len(df_portfolio) - 1:
                trough_idx = i if i < len(df_portfolio) else i - 1
                segment = df_portfolio.iloc[peak_idx : trough_idx + 1]
                trough_local_idx = segment["drawdown"].idxmin()
                dd_val = segment.loc[trough_local_idx, "drawdown"]
                drawdowns.append((peak_idx, trough_local_idx, dd_val))
                in_drawdown = False

    drawdowns.sort(key=lambda x: x[2])
    drawdowns = drawdowns[:top_n]
    ...
    for i, (pk_idx, trough_idx, dd_val) in enumerate(drawdowns, start=1):
        peak_date = df_portfolio.iloc[pk_idx]["date"]
        trough_date = df_portfolio.loc[trough_idx, "date"]
        peak_val = df_portfolio.iloc[pk_idx]["portfolio_val"]
        trough_val = df_portfolio.loc[trough_idx, "portfolio_val"]

        x0 = min(peak_date, trough_date)
        x1 = max(peak_date, trough_date)
        y0 = min(peak_val, trough_val)
        y1 = max(peak_val, trough_val)
        dd_str = f"{dd_val:.2%}"
        ...
        label_x = x0 + (x1 - x0) / 2 if x1 != x0 else x0
        label_y = y1 + (y1 - y0) * 0.1
        fig.add_annotation(
            x=label_x,
            y=label_y,
            text=f"Drawdown {i}: {dd_str}",
            showarrow=False,
            font=dict(size=12, color="#004466"),
            bgcolor="rgba(230, 245, 255, 0.85)",
            bordercolor="#004466",
            borderwidth=1,
            borderpad=4,
        )
    ...
    fig.update_layout(
        title=(
            "Largest Drawdowns<br>"
            "<sup>"
            "Shaded regions highlight the largest peak-to-trough losses. "
            "</sup>"
        ),
        xaxis_title="Date",
...
def plot_yearly_returns(df_portfolio):
    df_portfolio = df_portfolio.dropna(subset=["date", "ret"]).copy()
    df_portfolio["year"] = df_portfolio["date"].dt.year

    yearly_return = (
        df_portfolio.groupby("year")["ret"]
        .apply(lambda x: (1 + x).prod() - 1)
        .reset_index(name="annual_return")
    )

    yearly_return["annual_return_percent"] = yearly_return["annual_return"] * 100
    yearly_return["pos_neg"] = yearly_return["annual_return"].apply(
    ...
    fig = px.bar(
        yearly_return,
        x="year",
        y="annual_return_percent",
        text="annual_return_percent",
        color="pos_neg",
        color_discrete_map={"Positive": "#2CA02C", "Negative": "#D62728"},
        title="Yearly Returns on High-Risk Strategy",
        labels={"annual_return_percent": "Return (%)", "year": "Year", "pos_neg": "Return Type"}
    )
    ...
    max_val = yearly_return["annual_return_percent"].max()
    min_val = yearly_return["annual_return_percent"].min()
    buffer = (max_val - min_val) * 0.10 if (max_val - min_val) != 0 else 10
